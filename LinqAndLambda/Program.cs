...
namespace LinqAndLambda
{
    internal class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            var r2 = products.Where(p => p.Category.Name == "tool").Select(p => p.Name);
            Print("NAME OF PRODUCTS FROM TOOl", r2);

            var r3 = products.Where(p => p.Name[0] == 'C').Select(p => new {p.Name, p.Price, CategoryName = p.Category.Name});
            Print("NAMES STARTED WITH 'C' AND ANONYMOUS OBJECT", r3);

            var r4 = products.Where(p => p.Category.Tier == 1).OrderBy(p => p.Price).ThenBy(p => p.Name);
            Print("TIER 1 ORDENADO BY PRICE THEN BY NAME", r4);

            var r5 = r4.Skip(2).Take(4);
            Print("TIER 1 ORDENADO BY PRICE THEN BY NAME SKIP 2 TAKE 4", r5);
